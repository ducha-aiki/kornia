...
class SpatialGradient(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:  # type: ignore
        ...
        pd: int = self.kernel.size(-1) // 2
        channel_pd = self.order
        padded_inp: torch.Tensor = F.pad(F.pad(input, [pd, pd, pd, pd], 'replicate')[:, :, None], # noqa
                                         [0, 0, 0, 0, channel_pd, channel_pd], 'constant', 0)
